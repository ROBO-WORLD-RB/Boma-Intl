...
export default function ProductDetailClient({ initialProduct }: { initialProduct?: Product | null }) {
  ...
  useEffect(() => {
    async function fetchData() {
      // If we already have the product from props, only fetch reviews if needed
      if (initialProduct && product?.slug === slug) {
        if (reviews.length === 0) {
            try {
                const reviewsRes = await api.reviews.list(slug).catch(() => ({ data: [] }));
